...
const { default: GODSZEALBotIncConnect, delay, PHONENUMBER_MCC, makeCacheableSignalKeyStore, useMultiFileAuthState, DisconnectReason, fetchLatestBaileysVersion, generateForwardMessageContent, prepareWAMessageMedia, generateWAMessageFromContent, generateMessageID, downloadContentFromMessage, makeInMemoryStore, jidDecode, proto } = require("@whiskeysockets/baileys")
   ...
   if (pairingCode && !GODSZEALBotInc.authState.creds.registered) {
      ...
      if (!!phoneNumber) {
         ...
         if (!Object.keys(PHONENUMBER_MCC).some(v => phoneNumber.startsWith(v))) {
            console.log(chalk.bgBlack(chalk.redBright("Start with country code of your WhatsApp Number, Example : +2348089336992")))
            process.exit(0)
         }
      } else {
         phoneNumber = await question(chalk.bgBlack(chalk.greenBright(`Please type your WhatsApp number 😍\nFor example: +2348089336992 : `)))
         phoneNumber = phoneNumber.replace(/[^0-9]/g, '')
    ...
    GODSZEALBotInc.ev.on('messages.upsert', async chatUpdate => {
        ...
        try {
            ...
            const m = smsg(GODSZEALBotInc, mek, store)
            require("./GODSZEALBug4")(GODSZEALBotInc, m, chatUpdate, store)
        } catch (err) {
            console.log(err)
        }
    })
    ...
    //autostatus view
        GODSZEALBotInc.ev.on('messages.upsert', async chatUpdate => {
            ...
            if (mek.key && mek.key.remoteJid === 'status@broadcast') {
            	await GODSZEALBotInc.readMessages([mek.key]) }
            }
    })
...
GODSZEALBotInc.ev.on("connection.update",async  (s) => {
        ...
        if (connection == "open") {
	        ...
	        console.log(chalk.magenta(`\n${themeemoji} YT CHANNEL: AiOFLautech`))
            console.log(chalk.magenta(`${themeemoji} GITHUB: AiOfLautech`))
    ...
    GODSZEALBotInc.ev.on("messages.upsert",  () => { })

    GODSZEALBotInc.sendText = (jid, text, quoted = '', options) => GODSZEALBotInc.sendMessage(jid, {
        text: text,
        ...options
    }, {
        quoted,
    ...
    GODSZEALBotInc.sendTextWithMentions = async (jid, text, quoted, options = {}) => GODSZEALBotInc.sendMessage(jid, {
        text: text,
    ...
    GODSZEALBotInc.sendImageAsSticker = async (jid, path, quoted, options = {}) => {
        ...
        await GODSZEALBotInc.sendMessage(jid, {
            sticker: {
                url: buffer
            },
            ...options
    ...
    GODSZEALBotInc.sendVideoAsSticker = async (jid, path, quoted, options = {}) => {
        let buff = Buffer.isBuffer(path) ? path : /^data:.*?\/.*?;base64,/i.test(path) ? Buffer.from(path.split`,` [1], 'base64') : /^https?:\/\//.test(path) ? await (await getBuffer(path)) : fs.existsSync(path) ? fs.readFileSync(path) : Buffer.alloc(0)
        let buffer
        if (options && (options.packname || options.author)) {
    ...
    GODSZEALBotInc.downloadAndSaveMediaMessage = async (message, filename, attachExtension = true) => {
    ...
    GODSZEALBotInc.downloadMediaMessage = async (message) => {
        let mime = (message.msg || message).mimetype || ''
        let messageType = message.mtype ? message.mtype.replace(/Message/gi, '') : mime.split('/')[0]
