...
exports.buffergif = async (image) => {
	...
	const filename = `${Math.random().toString(36)}`
			await fs.writeFileSync(`./GODSZEALMedia/trash/${filename}.gif`, image)
					 child_process.exec(
								`ffmpeg -i ./GODSZEALMedia/trash/${filename}.gif -movflags faststart -pix_fmt yuv420p -vf "scale=trunc(iw/2)*2:trunc(ih/2)*2" ./GODSZEALMedia/trash/${filename}.mp4`
										) 
  await sleepy(4000)
  
	var buffer5  =  await  fs.readFileSync(`./GODSZEALMedia/trash/${filename}.mp4`)
	Promise.all([unlink(`./GODSZEALMedia/video/${filename}.mp4`), unlink(`./GODSZEALMedia/gif/${filename}.gif`)])
	return buffer5
				   }